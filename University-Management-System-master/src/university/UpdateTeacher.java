...
import static university.Conn.Connect;
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == b) {
            try {
                ...
                // Use PreparedStatement for SQL update to prevent SQL injection
                String str = "UPDATE teacher SET name=?, fathers_name=?, age=?, dob=?, address=?, phone=?, email=?, class_x=?, class_xii=?, aadhar=?, emp_id=?, course=?, dept=? WHERE emp_id=?";
                ...
                ps.setString(14, t12.getText());  // emp_id for WHERE clause
            ...
            try {
                ...
                // Use PreparedStatement for SQL query to prevent SQL injection
                String str = "SELECT * FROM teacher WHERE emp_id = ?";
                PreparedStatement ps = cn.prepareStatement(str);

                ps.setString(1, t12.getText()); // emp_id for WHERE clause
                ...
                if (rs.next()) {
                    ...
                    // Set values from the database result to the form fields
